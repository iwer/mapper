...
public class ServerFileHandler {
	...
	public void loadXML(String filename) {
		...
		for (int i = 0; i < pointsElement.length; i++) {
			...
			ServerPoint p = new ServerPoint(data.getParent(), data.server, id,
					x, y, data.isShowHelper());
		...
		for (int i = 0; i < edgesElement.length; i++) {
			...
			ServerEdge e = new ServerEdge(data.getParent(), data.server, id,
		...
		for (int i = 0; i < facesElement.length; i++) {
			...
			ServerFace f = new ServerFace(data.getParent(), data.server, id,
					data.isShowHelper());
